...
impl VideoService {
    ...
    pub async fn embed_metadata(dto: VideoEmbedDto) -> Result<(), String> {
        ...
        let first_performer = dto
            ...
            .unwrap_or("未知")
            .trim();
        let mut basename = format!("{}-{}", first_performer, dto.metadata.title.trim());
        ...
        let mut meta = vec![
            ...
            ("county".to_string(), dto.metadata.country),
...
async fn handle_entry(dir_entry: &DirEntry, pool: &SqlitePool, handle: &AppHandle) -> Result<()> {
