...
use crate::app::studio::StudioDetailVo;
...
impl<'tx, 'c> StudioRepo<'tx, 'c> {
    ...
    /// 列表查询
    pub async fn list_rows(&mut self) -> Result<Vec<StudioRow>> {
    ...
    /// 单行查询
    pub async fn find_by_id(&mut self, id: i64) -> Result<StudioRow> {
        let row = sqlx::query_as!(
            StudioRow,
            "SELECT id, name, url, image_blob FROM studios WHERE id = ?",
            id
        )
        .fetch_one(self.0.as_mut())
        .await?;
...
impl StudioRepo<'_, '_> {
    /// 裸池单行查询
    pub async fn find_by_id_direct(pool: &SqlitePool, id: i64) -> Result<StudioRow> {
        let row = sqlx::query_as!(
            StudioRow,
            "SELECT id, name, url, image_blob FROM studios WHERE id = ?",
            id
